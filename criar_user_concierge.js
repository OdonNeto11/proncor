...
const ROLE_CONCIERGE = 4;
const SETOR_AMBULATORIO = 2;

// Lista de Concierges limpa (sem duplicatas do WhatsApp)
const concierges = [
  { nome: 'Beatriz Dantas Da Silva', email: 'Beatriz.dantas@proncor.com.br' },
  { nome: 'Emilly Silva Soares', email: 'Emilly.silvas@proncor.com.br' },
  { nome: 'Stefani Paola Carmaem', email: 'stefani.carmaem@proncor.com.br' },
  { nome: 'Melissa Pereira de Azevedo', email: 'melissa.azevedo@proncor.com.br' }
];

async function importarConcierges() {
  console.log(`Iniciando importação de ${concierges.length} concierges...`);

  for (const usuario of concierges) {
    try {
      ...
      const { error: profileError } = await supabaseAdmin.from('profiles').insert({
        ...
        nome: usuario.nome,
        email: usuario.email,
        crm: null, // Concierges não possuem CRM
        is_active: true,
      ...
      // 3. Insere na tabela public.usuario_alocacoes (Apenas Ambulatório)
      const { error: alocacaoError } = await supabaseAdmin.from('usuario_alocacoes').insert({
        user_id: userId, 
        role_id: ROLE_CONCIERGE, 
        setor_id: SETOR_AMBULATORIO 
      });

      if (alocacaoError) throw new Error(`Erro Alocação: ${alocacaoError.message}`);

      // 4. Insere na tabela public.usuario_setores (Apenas Ambulatório)
      ...
      console.log(`✅ Acesso de Concierge (Ambulatório) configurado para ${usuario.nome}.`);

    } catch (err) {
      console.error(`❌ Falha na operação para ${usuario.email}:`, err.message);
    }
  }
  console.log('\n🚀 Importação de Concierges finalizada!');
}
