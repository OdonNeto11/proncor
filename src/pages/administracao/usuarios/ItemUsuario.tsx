...
export function ItemUsuario({ user, isCurrentUser, onEdit, onDelete, onToggleStatus }: ItemUsuarioProps) {
      ...
      <div className="flex items-center justify-end gap-2 md:w-72">
        ...
        {!isCurrentUser && (
          <>
            ...
            <button onClick={() => onToggleStatus(user.id, user.is_active)} className={`text-sm font-semibold flex items-center gap-1 px-3 py-2 rounded-lg transition-colors border w-28 justify-center ${user.is_active ? 'border-red-200 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20' : 'border-green-200 text-green-600 hover:bg-green-50 dark:hover:bg-green-900/20'}`}>
