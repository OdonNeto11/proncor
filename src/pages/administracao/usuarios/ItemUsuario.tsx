    ...
    <div className={`p-4 flex flex-col md:flex-row md:items-center justify-between gap-4 transition-colors ${user.is_active ? 'hover:bg-slate-50 dark:hover:bg-slate-800/50' : 'bg-red-50/50 dark:bg-red-900/10 opacity-75'}`}>
      <div className="flex-1">
        <div className="flex items-center gap-2">
          ...
          <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded-full ${user.is_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
      ...
      <div className="flex-1 md:text-center text-sm">
        ...
        <div className="text-slate-500 dark:text-slate-400 mt-1 flex flex-wrap md:justify-center items-center gap-1">
          <Building2 size={14}/> {user.usuario_setores?.map((us: any) => us.setores.nome).join(', ') || 'Sem Setor'}
        </div>
      </div>
