...
export function Agenda() {
  ...
  if (!podeVerPA) {
    return (
      <div className="max-w-4xl mx-auto p-12 text-center bg-white rounded-2xl border border-dashed border-gray-200 mt-10">
        <div className="w-20 h-20 bg-red-50 rounded-full flex items-center justify-center mx-auto mb-6">
          <AlertCircle size={48} className="text-red-400" />
        </div>
        <h2 className="text-2xl font-bold text-gray-800 mb-2">Acesso Negado</h2>
        <p className="text-gray-500">O seu perfil não tem permissão para visualizar a agenda do PA.</p>
        <Link to="/" className="inline-block mt-6 px-6 py-2 bg-gray-100 text-gray-600 font-bold hover:bg-gray-200 rounded-lg transition-colors">Voltar para Home</Link>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-6xl mx-auto animate-in fade-in duration-500 pb-20">
      ...
      <div className="flex items-center gap-6 mb-8 border-b border-gray-200 px-2">
        <Link to="/novo" className={`pb-3 text-sm font-bold border-b-2 transition-colors ${window.location.pathname === '/novo' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-400 hover:text-gray-600 hover:border-gray-300'}`}>Novo Agendamento</Link>
        <Link to="/agenda" className={`pb-3 text-sm font-bold border-b-2 transition-colors ${window.location.pathname === '/agenda' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-400 hover:text-gray-600 hover:border-gray-300'}`}>Ver Agenda</Link>
      </div>
                  ...
                  return (
                    <div key={item.id} onClick={() => abrirModal(item)} className="bg-white rounded-xl border p-4 shadow-sm hover:shadow-md cursor-pointer relative overflow-hidden group transition-all">
                        ...
                        <div className="flex gap-2 items-center mb-1">
                            {item.numero_atendimento && <span className="text-[10px] font-semibold text-gray-500">#{item.numero_atendimento}</span>}
                            {item.crm_responsavel && <span className="text-[9px] font-bold text-slate-500 bg-slate-100 px-1.5 py-0.5 rounded border border-slate-200">CRM {item.crm_responsavel}</span>}
                        </div>
...
actionButtons={
            selectedAgendamento.status?.agrupamento === 'pendente' ? (
              <>
                ...
                {/* BOTÃO COM TOM LARANJA RESTAURADO */}
                {podeReagendarPA && (
                  <button onClick={() => { setViewMode('reschedule'); setReagendarDate(new Date()); }} className="w-full bg-orange-50 text-orange-700 border border-orange-200 font-semibold py-2 rounded-lg hover:bg-orange-100 transition-colors flex items-center justify-center gap-2 text-sm">
      ...
      {selectedAgendamento && viewMode === 'edit' && (
        <Modal isOpen={true} onClose={() => setViewMode('details')} title={<span className="text-blue-600">Editando dados</span>}>
            <div className="space-y-4 animate-in slide-in-from-right-4 duration-300">
               ...
               <div>
                  <label className="text-sm font-semibold text-slate-700 mb-2 block">Procedimentos</label>
                  <div className="flex flex-wrap gap-2">
                    {OPCOES_PROCEDIMENTOS.map((proc) => {
                      const isSelected = editForm.procedimentos.includes(proc); 
                      return (
                        <button key={proc} onClick={() => toggleProcedimentoEdit(proc)} className={`px-3 py-1.5 rounded-full text-xs font-bold border transition-all flex items-center gap-1.5 ${isSelected ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-white text-slate-500 border-slate-200 hover:border-slate-300'}`}>
               ...
               {errorMsg && (
                 <div className="mb-2 p-3 bg-red-50 border border-red-200 text-red-600 rounded-xl text-sm flex items-center gap-2">
...
{/* 5. Modal de Reagendamento (Usando Componentes e Preservando Layout Grid) */}
        ...
        <Modal isOpen={true} onClose={() => setViewMode('details')} title={<span className="text-orange-600">Reagendamento</span>} maxWidth="2xl">
           <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
              <div className="bg-orange-50 p-4 rounded-xl border border-orange-100">
                  <p className="text-sm text-orange-800 text-center font-bold">Selecione a nova data e horário</p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                      ...
                      <div className="relative">
                          ...
                          <DatePicker selected={reagendarDate} onChange={(d: Date | null) => setReagendarDate(d)} minDate={new Date()} locale="pt-BR" dateFormat="dd/MM/yyyy" placeholderText="Selecione o dia" popperPlacement="bottom-start" className="w-full pl-10 pr-3 py-3 rounded-xl border border-slate-200 text-sm focus:ring-4 focus:ring-blue-500/10 outline-none bg-white" onFocus={(e) => e.target.blur()} />
                      </div>
                  </div>
                  <div>
                     ...
                     {!reagendarDate ? (
                        ...
                        <div className="grid grid-cols-3 gap-2 max-h-48 overflow-y-auto pr-1">
                            {horariosDisponiveis.map((horario) => { 
                                ...
                                return (
                                    <button key={horario} type="button" disabled={isDisabled} onClick={() => handleSelectRescheduleTime(horario)} className={`py-2 px-1 rounded-lg text-xs font-bold border transition-all ${isDisabled ? 'bg-slate-50 text-slate-300 border-slate-100 cursor-not-allowed' : isSelected ? 'bg-blue-600 text-white border-blue-600 shadow-md transform scale-105' : 'bg-white text-slate-600 border-slate-200 hover:border-blue-400 hover:text-blue-600 hover:shadow-sm'}`}>
                                        {horario}
                                    </button>
                                );
                            })}
              ...
              {/* BOTÕES RESTAURADOS PARA O SEU LAYOUT ORIGINAL */}
              <div className="flex gap-2 pt-2">
                  <button onClick={() => setViewMode('details')} className="flex-1 bg-gray-100 text-gray-600 py-3 rounded-xl text-sm font-bold hover:bg-gray-200 transition-colors">Voltar</button>
                  <button onClick={confirmarReagendamento} className="flex-1 bg-orange-600 text-white py-3 rounded-xl text-sm hover:bg-orange-700 font-bold transition-colors shadow-sm">Confirmar</button>
              </div>
           </div>
        </Modal>
      )}
